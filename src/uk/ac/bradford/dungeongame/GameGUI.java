...
public class GameGUI extends JFrame {
    ...
    /**
     * Method to create and initialize components for displaying elements of the
     * game on the screen.
     */
    private void initGUI() {
        add(canvas = new Canvas());     //adds canvas to this frame
        setTitle("Dungeon - The Last Hope");
        setSize(815, 615);
     ...
     * @param player An Entity object with the type EntityType.PLAYER and the
     * number numberOfPlayer.FIRST_PLAYER. This object is used to draw the first
     * player in the right dungeon tile and display its health. Null can be
     * passed for this argument, in which case no player will be drawn.
     * @param secondPlayer An Entity object with the type EntityType.PLAYER and
     * the number numberOfPlayer_SECOND_PLAYER. This object is used to draw the
     * second player in the right dungeon tile and display its health. Null can
     * be passed for this argument, in which case no player will be drawn.
     ...
     */
    public void updateDisplay(TileType[][] tiles, Entity player, Entity secondPlayer, Entity[] monsters) {
        canvas.update(tiles, player, secondPlayer, monsters);
    }
...
class Canvas extends JPanel {
    ...
    private BufferedImage woodenChest;
    private BufferedImage sapphireChest;
    private BufferedImage rubyChest;
    private BufferedImage trapChest;

    private TileType[][] currentTiles;  //the current 2D array of tiles to display
    private Entity currentPlayer;       //the current player object to be drawn
    private Entity currentSecondPlayer; //the second player object to be drawn
    private Entity[] currentMonsters;   //the current array of monsters to draw
    ...
    private void loadTileImages() {
        try {
            ...
            secondPlayer = ImageIO.read(new File("assets/secondPlayer.png"));
            assert secondPlayer.getHeight() == GameGUI.TILE_HEIGHT
                    && secondPlayer.getWidth() == GameGUI.TILE_WIDTH;
            ...
            woodenChest = ImageIO.read(new File("assets/wooden chest.png"));
            assert woodenChest.getHeight() == GameGUI.TILE_HEIGHT
                    && woodenChest.getWidth() == GameGUI.TILE_WIDTH;
            sapphireChest = ImageIO.read(new File("assets/sapphire chest.png"));
            assert sapphireChest.getHeight() == GameGUI.TILE_HEIGHT
                    && sapphireChest.getWidth() == GameGUI.TILE_WIDTH;
            rubyChest = ImageIO.read(new File("assets/ruby chest.png"));
            assert rubyChest.getHeight() == GameGUI.TILE_HEIGHT
                    && rubyChest.getWidth() == GameGUI.TILE_WIDTH;
            trapChest = ImageIO.read(new File("assets/trap chest.png"));
            assert trapChest.getHeight() == GameGUI.TILE_HEIGHT
                    && trapChest.getWidth() == GameGUI.TILE_WIDTH;
        } catch (IOException e) {
     ...
     * dungeon
     * @param player The current first player object, used to draw the first
     * player and its health
     * @param secondPlayer The current second player object, used to draw the
     * second player and its health
     * @param mon The array of monsters to display them and their health
     */
    public void update(TileType[][] t, Entity player, Entity secondPlayer, Entity[] mon) {
        currentTiles = t;
        currentPlayer = player;
        currentSecondPlayer = secondPlayer;
        currentMonsters = mon;
        repaint();
    }
     ...
     * level tiles, the players and the monsters. If the tiles, player or monster
    ...
    private void drawDungeon(Graphics g) {
                    ...
                    if (currentTiles[i][j] != null) {   //checks a tile exists
                        switch (currentTiles[i][j]) {
                            ...
                            case STAIRS:
                                ...
                                break;
                            case WOODEN_CHEST:
                                g2.drawImage(woodenChest, i * GameGUI.TILE_HEIGHT, j * GameGUI.TILE_HEIGHT, null);
                                break;
                            case SAPPHIRE_CHEST:
                                g2.drawImage(sapphireChest, i * GameGUI.TILE_HEIGHT, j * GameGUI.TILE_HEIGHT, null);
                                break;
                            case RUBY_CHEST:
                                g2.drawImage(rubyChest, i * GameGUI.TILE_HEIGHT, j * GameGUI.TILE_HEIGHT, null);
                                break;
                        }
                    }
                }
        ...
        if (currentPlayer != null) {
            ...
            drawShieldBar(g2, currentPlayer);
        }
        if (currentSecondPlayer != null) {
            g2.drawImage(secondPlayer, currentSecondPlayer.getX() * GameGUI.TILE_WIDTH, currentSecondPlayer.getY() * GameGUI.TILE_HEIGHT, null);
            drawHealthBar(g2, currentSecondPlayer);
            drawShieldBar(g2, currentSecondPlayer);
        }
    }
    ...
    private void drawHealthBar(Graphics2D g2, Entity e) {
        ...
        g2.fill(new Rectangle2D.Double(e.getX() * GameGUI.TILE_WIDTH, e.getY() * GameGUI.TILE_HEIGHT + 29, GameGUI.TILE_WIDTH, GameGUI.BAR_HEIGHT));
        g2.setColor(Color.GREEN);
        g2.fill(new Rectangle2D.Double(e.getX() * GameGUI.TILE_WIDTH, e.getY() * GameGUI.TILE_HEIGHT + 29, GameGUI.TILE_WIDTH * remainingHealth, GameGUI.BAR_HEIGHT));
    }
    
    /**
     * Draws a shield bar for the given Entity at the bottom of the tile that
     * the Entity is located in.
     * 
     * @param g2 The graphics object to use for drawn
     * @param e The Entity that the shield bar will be drawn for
     */
    private void drawShieldBar(Graphics2D g2, Entity e) {
        double remainingShield = (double) e.getShield() / (double) e.getMaxHealth();
        g2.setColor(Color.BLUE);
        g2.fill(new Rectangle2D.Double(e.getX() * GameGUI.TILE_WIDTH, e.getY() * GameGUI.TILE_WIDTH + 29, GameGUI.TILE_WIDTH * remainingShield, GameGUI.BAR_HEIGHT));
    }
}
